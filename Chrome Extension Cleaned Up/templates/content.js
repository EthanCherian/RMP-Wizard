...
function setPredictions(reloadSentimentsOrNot) {
    $(document).ready(function() {
        ...
        for(var i = 0; i < els.length; i++) {
            ...
            if(reloadSentimentsOrNot || (getLine != null && checkSentiment == null))
        ...
        $.ajax({
            url:"http://127.0.0.1:5000/processAndSendTemp",
            type:"POST",
            contentType:"application/json",
            ...
            success: function(response) {
                ...
                for(var i = 0; i < els.length; i++) {
                    ...
                    // console.log(sentiments[1]);
                    if(getLine != null && checkSentiment != null) {
                        var modifySentiment = els[i].querySelector('div[class="insertedSentiment sentiment"]');
                        checkSentiment.innerHTML = sentiments[0][sentimentsCounter] + ' text detected';

                        // console.log(sentiments[1][sentimentsCounter]);
                        var modifyConf = forConfRatings[i].querySelector('div[class="insertConfNum conf_rating_num"]');
                        // console.log(modifyConf);
                        modifyConf.innerHTML = sentiments[1][sentimentsCounter] + '%';
...
$(document).ready(function() {
    ...
    let courseWatcher = document.getElementsByClassName(' css-n2irg-placeholder')[0];


    observer2 = new MutationObserver(RMPCallback2);
    observer = new MutationObserver(RMPCallback);

    function RMPCallback(mutations) {
        setPredictions(new Boolean(false));
    }

    function RMPCallback2(mutations) {
        setPredictions(new Boolean(true));
    }
    ...
    if(document.querySelector('div[class=" css-n2irg-placeholder"]') != null) {
        observer2.observe(courseWatcher, {attributes:true});
    }
